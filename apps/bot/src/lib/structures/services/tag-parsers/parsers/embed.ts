import { ColorResolvable, resolveColor } from "seyfert/lib/common";
import { APIEmbed } from "seyfert/lib/types";
import { Awaitable, BaseParser, Context, IParser, split } from "tagscript";
 ...
 * ```
 * @copyright https://github.com/imranbarbhuiya/TagScript/blob/main/packages/tagscript-plugin-discord/src/lib/Parsers/Embed.ts (adapted to seyfert)
 */
export class EmbedParser extends BaseParser implements IParser {
	...
	public async parse(ctx: Context) {
		if (!ctx.tag.parameter)
			return this.returnEmbed(ctx, await this.parseEmbedJSON(ctx.tag.payload!));

		if (ctx.tag.payload!.startsWith("{") && ctx.tag.payload!.endsWith("}"))
			return this.returnEmbed(ctx, {
				[ctx.tag.parameter]: JSON.parse(ctx.tag.payload!) as unknown,
			});
